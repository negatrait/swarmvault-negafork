...
func handleAgents() {
	...
	case "getAgentInstallStatus":
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "Error from GetAgentInstallStatus: %v\n", err)
			os.Exit(1)
		}
		if err := json.NewEncoder(os.Stdout).Encode(result); err != nil {
			os.Exit(1)
	...
	case "installConfiguredAgents":
		...
		result, err := agents.InstallConfiguredAgents(args.RootDir)
		if err != nil {
			fmt.Fprintf(os.Stderr, "Error from InstallConfiguredAgents: %v\n", err)
			os.Exit(1)
		}
		if err := json.NewEncoder(os.Stdout).Encode(result); err != nil {
			os.Exit(1)
