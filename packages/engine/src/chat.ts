...
export async function askChatSession(rootDir: string, options: AskChatOptions): Promise<AskChatResult> {
  ...
  if (process.env.USE_GO_PORT === "true") {
    // biome-ignore lint/suspicious/noExplicitAny: Required for bridging untyped JSON to TS types
    const prepareResult = await runGoSidecar<any>("chat", {
      action: "askChatSessionPrepare",
      args: { rootDir, options, now }
    });
