...
func ResolveLargeRepoDefaults(input ResolveLargeRepoDefaultsInput) ResolvedLargeRepoDefaults {
	nodeCount := int(math.Max(0, math.Floor(input.NodeCount)))
	totalCommunities := 0
	if input.TotalCommunities != nil {
		totalCommunities = int(math.Max(0, math.Floor(*input.TotalCommunities)))
	}
	...
	defaultSimilarityEdgeCap := int(math.Min(SimilarityEdgeCapMax, math.Max(0, float64(SimilarityEdgeCapPerNode*nodeCount))))
	...
	defaultFoldBelow := int(math.Max(MinFoldBelow, math.Ceil(float64(totalCommunities)/50.0)))
