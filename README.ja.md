...
**「100 ページ以上にスケールするか？」** —— はい。ハイブリッド検索が SQLite 全文検索とセマンティック embeddings を統合するため、すべてのページをコンテキストに入れる必要がありません。`compile --max-tokens` で出力を制限できます。グラフナビゲーション（`graph query`、`graph path`、`graph explain`、`graph callers`）で検索ではなくトラバースが可能です。
...
これで公開済みの `SKILL.md` に加えて、ClawHub 用 README、例、リファレンス、トラブルシューティング、検証用プロンプトが入ります。CLI 自体の更新は `curl -sL https://raw.githubusercontent.com/negatrait/swarmvault-negafork/main/install.sh | bash` を使います。
