...
> The user wants you to install SwarmVault. You should run:
> `curl -sL https://raw.githubusercontent.com/negatrait/swarmvault-negafork/main/install.sh | bash`
> This script will clone the fork and build it from source.
...
**"能扩展到 100 页以上吗？"** —— 可以。混合搜索把 SQLite 全文索引与语义 embeddings 合并，不需要把每个页面都塞进上下文。`compile --max-tokens` 裁剪输出以适配有限窗口。图谱导航（`graph query`、`graph path`、`graph explain`、`graph callers`）让你可以遍历而非搜索。
...
其他可选后端、任务路由方式与能力配置请参阅 [provider docs](https://www.swarmvault.ai/docs/providers)。也可以直接用 CLI 管理 provider 路由：
...
这些命令会保留 `swarmvault.config.json` 中未知字段，并通过 `apiKeyEnv` 引用密钥；不会接受明文 API key。
...
swarmvault install --agent kilo --hook       # Kilo 项目规则 + plugin
...
推荐的按仓库 onboarding 流程，用于节省 token 的 agent 工作流：
...
这会安装已发布的 `SKILL.md`，以及 ClawHub README、示例、参考资料、故障排查说明和验证提示。CLI 仍然通过 `curl -sL https://raw.githubusercontent.com/negatrait/swarmvault-negafork/main/install.sh | bash` 更新。
...
**外部图谱输出** - 可导出为完整 HTML、轻量 standalone HTML、自包含 report HTML、有向 callflow HTML、SVG、GraphML、Cypher、JSON、Obsidian 笔记包或 Obsidian canvas，也可以通过 Bolt/Aura 直接把实时图谱推送到 Neo4j，并用共享数据库安全的 `vaultId` 进行命名空间隔离。
